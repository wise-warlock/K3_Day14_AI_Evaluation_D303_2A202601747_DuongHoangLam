...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = f"Question: {question}\nAnswer: {answer}\nRubric: {rubric_str}\nScore response and return JSON format."
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(scores_batch) >= 2:
            s1 = scores_batch[0].get("scores", {})
            s2 = scores_batch[-1].get("scores", {})
            avg1 = sum(s1.values()) / len(s1) if s1 else 0.5
            avg2 = sum(s2.values()) / len(s2) if s2 else 0.5
            if avg1 - avg2 > 0.2:
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list[EvalResult], baseline_results: list[EvalResult]) -> dict[str, Any]:
        ...
        """
        def calc_avg(res_list: list[EvalResult], attr: str) -> float:
            if not res_list:
                return 0.0
            return sum(getattr(r, attr) for r in res_list) / len(res_list)

        new_f = calc_avg(new_results, "faithfulness")
        new_r = calc_avg(new_results, "relevance")
        new_c = calc_avg(new_results, "completeness")

        base_f = calc_avg(baseline_results, "faithfulness")
        base_r = calc_avg(baseline_results, "relevance")
        base_c = calc_avg(baseline_results, "completeness")
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        below_half = sum(1 for s in (failure.faithfulness, failure.relevance, failure.completeness) if s < 0.5)
        if below_half > 1:
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant", 0) > 0:
            suggestions.append("Refine system prompt and query expansion to improve answer relevance")
